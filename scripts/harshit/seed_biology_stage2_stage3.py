#!/usr/bin/env python3
"""Seed Harshit Biology Unit 1 Stage 2/3 ncert_source.json and exercise_bank.json."""
...
from scripts.harshit.biology_stage2_stage3_data import UNIT_DATA  # noqa: E402


def _write_ncert(unit_id: int, pack: dict) -> Path:
    out = ROOT / "HarshitBiology" / f"unit{unit_id}" / "ncert_source.json"
    payload = {
        "meta": pack["meta"],
...
def _write_exercise_bank(unit_id: int, pack: dict) -> Path:
    out = ROOT / "HarshitBiology" / f"unit{unit_id}" / "exercise_bank.json"
