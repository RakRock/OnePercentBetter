#!/usr/bin/env python3
"""Build Harshit Physics Unit 2 question bank (200 MCQs from concept cards)."""
...
UNIT_ID = 2
TARGET_TOTAL = 200
BANK_PATH = ROOT / "HarshitPhysics" / "unit2" / "question_bank.json"
...
def _q_confusion(concept: dict, day_id: int, concepts: list[dict], level: str, seq: int) -> dict:
    ...
    wrong = [confusion, "There are no common mistakes for this topic.", "Any answer is acceptable in exams."]
    options, answer = _shuffle_options(correct, wrong)
    return {
        "id": f"{PREFIX}_d{day_id}_{level.lower()}_{seq:03d}",
