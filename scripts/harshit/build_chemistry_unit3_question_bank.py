#!/usr/bin/env python3
"""Build Harshit Chemistry Unit 3 question bank (200 MCQs from concept cards)."""
...
TARGET_TOTAL = 200
BANK_PATH = ROOT / "HarshitChemistry" / "unit3" / "question_bank.json"
...
def _q_recall(concept: dict, day_id: int, concepts: list[dict], level: str, seq: int) -> dict:
    ...
    return {
        "id": f"u3_d{day_id}_{level.lower()}_{seq:03d}",
        ...
        "level": level,
        "category": f"u3_d{day_id}_{level}",
        "category_label": hpt.format_topic_level_label(3, day_id, level),
        "source": "bank",
...
def _questions_for_day(day_id: int, count: int) -> list[dict]:
    concepts = hpc.concepts_for_day(day_id, unit_id=3)
    if not concepts:
        return []
    out: list[dict] = []
    seq = 1
...
def build_bank(*, seed: int = 42) -> dict:
    random.seed(seed)
    days = [d for d in hpc.list_days(stage=1, unit_id=3) if d.get("active")]
    ...
    return {
        "meta": {
            ...
            "total": len(unique),
            "target": TARGET_TOTAL,
            "ncert": "Class 10 Science, Chapter 3 — Acids, Bases and Salts",
