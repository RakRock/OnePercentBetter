...
def init_db():
    ...
    with get_connection() as conn:
        conn.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS arjun_course3_week_config (
                unit_id INTEGER PRIMARY KEY,
                week_label TEXT NOT NULL DEFAULT '',
                config_json TEXT NOT NULL DEFAULT '{}',
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            ...
            CREATE TABLE IF NOT EXISTS arjun_edgenuity_course3_week_config (
                unit_id INTEGER PRIMARY KEY,
                week_label TEXT NOT NULL DEFAULT '',
                config_json TEXT NOT NULL DEFAULT '{}',
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            ...
            CREATE TABLE IF NOT EXISTS harshit_biology_concept_status (
                user_id INTEGER NOT NULL,
                unit_id INTEGER NOT NULL,
                concept_id TEXT NOT NULL,
                viewed INTEGER NOT NULL DEFAULT 0,
            ...
            CREATE TABLE IF NOT EXISTS harshit_biology_day_status (
                user_id INTEGER NOT NULL,
                unit_id INTEGER NOT NULL,
                day_id INTEGER NOT NULL,
                status TEXT NOT NULL DEFAULT 'not_started',
            ...
            CREATE TABLE IF NOT EXISTS harshit_biology_mcq_attempts (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                user_id INTEGER NOT NULL,
                unit_id INTEGER NOT NULL,
                day_id INTEGER NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS harshit_biology_misconceptions (
                user_id INTEGER NOT NULL,
                unit_id INTEGER NOT NULL,
                category TEXT NOT NULL,
                count INTEGER NOT NULL DEFAULT 0,
            ...
            CREATE TABLE IF NOT EXISTS harshit_biology_week_config (
                unit_id INTEGER PRIMARY KEY,
                week_label TEXT NOT NULL DEFAULT '',
                config_json TEXT NOT NULL DEFAULT '{}',
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
...
def merge_harshit_concept_progress_from_sheet(
    ...
    *,
    ...
    if module not in ("physics", "chemistry", "biology"):
        return
...
def save_arjun_course3_week_config(
    ...
    *,
    ...
    with get_connection() as conn:
        conn.execute(
            """INSERT INTO arjun_course3_week_config (unit_id, week_label, config_json, updated_at)
               VALUES (?, ?, ?, CURRENT_TIMESTAMP)
...
def save_arjun_edgenuity_course3_week_config(
    ...
    *,
    ...
    with get_connection() as conn:
        conn.execute(
            """INSERT INTO arjun_edgenuity_course3_week_config (unit_id, week_label, config_json, updated_at)
               VALUES (?, ?, ?, CURRENT_TIMESTAMP)
               ON CONFLICT(unit_id) DO UPDATE SET
                 week_label = excluded.week_label,
                 config_json = excluded.config_json,
...
def save_harshit_chemistry_concept_status(
    user_id: int,
    *,
    unit_id: int,
    concept_id: str,
    ...
    with get_connection() as conn:
        row = conn.execute(
            """SELECT simpler_requests, example_requests, viewed, marked_review
               FROM harshit_chemistry_concept_status
               WHERE user_id = ? AND unit_id = ? AND concept_id = ?""",
            (user_id, unit_id, concept_id),
        ).fetchone()
        simpler = int(row["simpler_requests"]) if row else 0
        ...
        review_val = 1 if marked_review else (int(row["marked_review"]) if row else 0)
        conn.execute(
            """INSERT INTO harshit_chemistry_concept_status
               (user_id, unit_id, concept_id, viewed, marked_review, simpler_requests,
                example_requests, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
               ON CONFLICT(user_id, unit_id, concept_id) DO UPDATE SET
                 viewed = MAX(harshit_chemistry_concept_status.viewed, excluded.viewed),
                 marked_review = excluded.marked_review,
                 simpler_requests = excluded.simpler_requests,
                 example_requests = excluded.example_requests,
                 updated_at = CURRENT_TIMESTAMP""",
...
def get_harshit_chemistry_viewed_concepts(user_id: int, *, unit_id: int) -> list[str]:
    with get_connection() as conn:
        rows = conn.execute(
            """SELECT concept_id FROM harshit_chemistry_concept_status
               WHERE user_id = ? AND unit_id = ? AND viewed = 1""",
            (user_id, unit_id),
        ).fetchall()
    return [r["concept_id"] for r in rows]
...
def get_harshit_chemistry_review_concepts(user_id: int, *, unit_id: int) -> list[str]:
    with get_connection() as conn:
        rows = conn.execute(
            """SELECT concept_id FROM harshit_chemistry_concept_status
               WHERE user_id = ? AND unit_id = ? AND marked_review = 1""",
...
def update_harshit_chemistry_day_status(
    user_id: int,
    *,
    unit_id: int,
    day_id: int,
    ...
    with get_connection() as conn:
        conn.execute(
            """INSERT INTO harshit_chemistry_day_status
               (user_id, unit_id, day_id, status, concepts_viewed, concepts_total, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
               ON CONFLICT(user_id, unit_id, day_id) DO UPDATE SET
                 status = excluded.status,
...
def get_harshit_chemistry_week_config(unit_id: int) -> dict:
    with get_connection() as conn:
        row = conn.execute(
            "SELECT week_label, config_json FROM harshit_chemistry_week_config WHERE unit_id = ?",
            (unit_id,),
        ).fetchone()
    if not row:
        return {
...
def save_harshit_chemistry_week_config(
    unit_id: int,
    week_label: str,
    topics: list[dict],
    *,
    ...
    with get_connection() as conn:
        conn.execute(
            """INSERT INTO harshit_chemistry_week_config (unit_id, week_label, config_json, updated_at)
               VALUES (?, ?, ?, CURRENT_TIMESTAMP)
               ON CONFLICT(unit_id) DO UPDATE SET
                 week_label = excluded.week_label,
                 config_json = excluded.config_json,
...
def save_harshit_chemistry_mcq_attempt(
    user_id: int,
    *,
    unit_id: int,
    day_id: int,
    ...
    with get_connection() as conn:
        conn.execute(
            """INSERT INTO harshit_chemistry_mcq_attempts
               (user_id, unit_id, day_id, question_id, selected, correct,
                misconception, concept_reviewed, retry_correct)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
        ...
        if misconception and not correct:
            conn.execute(
                """INSERT INTO harshit_chemistry_misconceptions (user_id, unit_id, category, count, last_seen)
                   VALUES (?, ?, ?, 1, CURRENT_TIMESTAMP)
                   ON CONFLICT(user_id, unit_id, category) DO UPDATE SET
                     count = count + 1,
                     last_seen = CURRENT_TIMESTAMP""",
...
def save_harshit_biology_concept_status(
    user_id: int,
    *,
    unit_id: int,
    concept_id: str,
    ...
    with get_connection() as conn:
        row = conn.execute(
            """SELECT simpler_requests, example_requests, viewed, marked_review
               FROM harshit_biology_concept_status
               WHERE user_id = ? AND unit_id = ? AND concept_id = ?""",
            (user_id, unit_id, concept_id),
        ).fetchone()
        simpler = int(row["simpler_requests"]) if row else 0
        ...
        review_val = 1 if marked_review else (int(row["marked_review"]) if row else 0)
        conn.execute(
            """INSERT INTO harshit_biology_concept_status
               (user_id, unit_id, concept_id, viewed, marked_review, simpler_requests,
                example_requests, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
               ON CONFLICT(user_id, unit_id, concept_id) DO UPDATE SET
                 viewed = MAX(harshit_biology_concept_status.viewed, excluded.viewed),
                 marked_review = excluded.marked_review,
                 simpler_requests = excluded.simpler_requests,
                 example_requests = excluded.example_requests,
                 updated_at = CURRENT_TIMESTAMP""",
    ...
    _push_harshit_concept_progress(
        user_id, module="biology", unit_id=unit_id, concept_id=concept_id
    )


def get_harshit_biology_viewed_concepts(user_id: int, *, unit_id: int) -> list[str]:
    with get_connection() as conn:
        rows = conn.execute(
            """SELECT concept_id FROM harshit_biology_concept_status
               WHERE user_id = ? AND unit_id = ? AND viewed = 1""",
            (user_id, unit_id),
        ).fetchall()
    return [r["concept_id"] for r in rows]
...
def get_harshit_biology_review_concepts(user_id: int, *, unit_id: int) -> list[str]:
    with get_connection() as conn:
        rows = conn.execute(
            """SELECT concept_id FROM harshit_biology_concept_status
               WHERE user_id = ? AND unit_id = ? AND marked_review = 1""",
...
def update_harshit_biology_day_status(
    user_id: int,
    *,
    unit_id: int,
    day_id: int,
    ...
    with get_connection() as conn:
        conn.execute(
            """INSERT INTO harshit_biology_day_status
               (user_id, unit_id, day_id, status, concepts_viewed, concepts_total, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
               ON CONFLICT(user_id, unit_id, day_id) DO UPDATE SET
                 status = excluded.status,
    ...
    _push_harshit_day_progress(user_id, module="biology", unit_id=unit_id, day_id=day_id)


def get_harshit_biology_week_config(unit_id: int) -> dict:
    with get_connection() as conn:
        row = conn.execute(
            "SELECT week_label, config_json FROM harshit_biology_week_config WHERE unit_id = ?",
            (unit_id,),
        ).fetchone()
    if not row:
        return {
...
def save_harshit_biology_week_config(
    unit_id: int,
    week_label: str,
    topics: list[dict],
    *,
    ...
    with get_connection() as conn:
        conn.execute(
            """INSERT INTO harshit_biology_week_config (unit_id, week_label, config_json, updated_at)
               VALUES (?, ?, ?, CURRENT_TIMESTAMP)
               ON CONFLICT(unit_id) DO UPDATE SET
                 week_label = excluded.week_label,
                 config_json = excluded.config_json,
...
def save_harshit_biology_mcq_attempt(
    user_id: int,
    *,
    unit_id: int,
    day_id: int,
    ...
    with get_connection() as conn:
        conn.execute(
            """INSERT INTO harshit_biology_mcq_attempts
               (user_id, unit_id, day_id, question_id, selected, correct,
                misconception, concept_reviewed, retry_correct)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
        ...
        if misconception and not correct:
            conn.execute(
                """INSERT INTO harshit_biology_misconceptions (user_id, unit_id, category, count, last_seen)
                   VALUES (?, ?, ?, 1, CURRENT_TIMESTAMP)
                   ON CONFLICT(user_id, unit_id, category) DO UPDATE SET
                     count = count + 1,
                     last_seen = CURRENT_TIMESTAMP""",
