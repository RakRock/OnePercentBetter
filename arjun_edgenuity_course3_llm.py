"""LLM-generated Edgenuity Course 3 unit practice questions via xAI Grok."""
...
def _system_prompt(
    *,
    ...
    return f"""You are a Grade 8 math tutor creating Edgenuity-style multiple-choice practice for an 11-year-old student named Arjun.
...
RULES:
- Grade 8 difficulty; match Edgenuity exam wording (clear, concrete scenarios).
...
- Text-only questions (no images) — describe graphs/tables in words when needed, e.g. "A line passes through (0, 5) and (2, 9)…"
...
def _to_session_question(q: dict, unit_id: int, categories: dict) -> dict:
    ...
    return {
        "id": f"ec3_llm_u{unit_id}_{cat}_{stamp}_{random.randint(100, 999)}",
        "category": cat,
        "question": q["question"],
        "options": q["options"],
        "answer": q["answer"],
