"""Harshit Chemistry — Stage 2 structured MCQ sessions (days 17–20)."""
...
def _ss_key(name: str, unit_id: int | None = None) -> str:
    ...
    return f"hc_mcq_u{uid}_{name}"


def _mcq_state_key(day_id: int, unit_id: int) -> str:
    return f"hc_mcq_state_{unit_id}_{day_id}"
...
def _open_concept_by_id(concept_id: str, unit_id: int) -> None:
    ...
    key = f"hc_concept_{unit_id}_{day_id}"
...
def render_stage2_home(*, stage1_done: bool, unit_id: int = 1) -> None:
    ...
    for session in hpc.list_mcq_sessions(unit_id):
        ...
        with st.container(border=True):
            ...
            if st.button("Start session", key=f"hc_stage2_start_{unit_id}_{day_id}", type="primary"):
                st.session_state.hc_unit_id = unit_id
                st.session_state.hc_mcq_day_id = day_id
                ...
                st.session_state.current_page = "harshit_chemistry_mcq"
                st.rerun()
...
def render_mcq_session() -> None:
    ...
    day_id = int(st.session_state.get("hc_mcq_day_id", 17))
    ...
    for idx, opt in enumerate(q["options"]):
        with cols[idx % 2]:
            if st.button(str(opt), key=f"hc_mcq_u{unit_id}_d{day_id}_q{current}_{idx}", use_container_width=True):
                picked = idx
...
def _render_remediation(payload: dict, unit_id: int, day_id: int, state: hcs.MCQSessionState) -> None:
    ...
    if concept_id:
        if st.button("Review related concept", key=f"hc_mcq_review_concept_{unit_id}"):
            _open_concept_by_id(concept_id, unit_id)
            st.session_state.pop(_ss_key("remediation", unit_id), None)
            st.rerun()

    if st.button("Continue to next question →", type="primary", key=f"hc_mcq_continue_{unit_id}"):
        st.session_state.pop(_ss_key("remediation", unit_id), None)
        current = int(st.session_state.get(_ss_key("current", unit_id), 0))
        st.session_state[_ss_key("current", unit_id)] = current + 1
        st.rerun()
