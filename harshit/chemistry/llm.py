"""Generate Chemistry Unit 1 MCQs via xAI Grok, seeded from the 200-question bank."""
...
def _batch_system_prompt(unit_id: int) -> str:
    ...
    if unit_id == 2:
        topics = """SOURCE: Questions MUST align with NCERT Class 10 Ch 2 — Acids, Bases and Salts:
indicators (litmus, turmeric, methyl orange, phenolphthalein), acid/base reactions with metals,
carbonates and hydrogencarbonates, neutralisation, metal/non-metal oxides, H⁺ and OH⁻ ions in water,
pH scale and everyday pH, salts and their families, chemicals from common salt, water of crystallisation."""
    elif unit_id == 3:
        topics = """SOURCE: Questions MUST align with NCERT Class 10 Ch 3 — Metals and Non-metals:
physical properties (lustre, malleability, ductility, conductivity), reactions with O₂, water, acids,
displacement reactions, reactivity series, ionic bonding and properties of ionic compounds,
extraction of metals (ores, calcination, roasting, reduction, electrolysis), corrosion and prevention."""
    elif unit_id == 4:
        topics = """SOURCE: Questions MUST align with NCERT Class 10 Ch 4 — Carbon and its Compounds:
covalent bonding and Lewis structures, allotropes (diamond, graphite, fullerene), saturated/unsaturated
hydrocarbons, homologous series and functional groups, IUPAC naming, combustion and oxidation,
addition and substitution reactions, ethanol and ethanoic acid, soaps and detergents, catenation."""
    else:
        topics = """SOURCE: Questions MUST align with NCERT Class 10 Ch 1 — Chemical Reactions and Equations:
chemical vs physical change, signs of reaction, word/symbol/balanced equations, conservation of mass,
combination, decomposition (thermal/electrolytic/photolytic), displacement, double displacement,
oxidation and reduction, corrosion, rancidity."""
    chapter_ref = umeta.get("chapter_ref", "NCERT Class 10 Chemistry")
    return f"""You are an NCERT Class 10 Science (Chemistry) tutor creating multiple-choice practice
...
- Short explanation (1-2 sentences) referencing the chemistry idea.
        ...
        validated.append(
            {
                ...
                "chapter_ref": str(q.get("chapter_ref", hpc.unit_meta(unit_id).get("chapter_ref", "NCERT Chemistry"))).strip(),
                "source": "chapter_llm",
            }
        )
    return validated
