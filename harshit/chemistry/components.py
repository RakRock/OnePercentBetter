"""Chemistry concept visuals — labeled SVG (primary) + optional cached HF illustrations."""
...
def inject_chemistry_styles(unit_id: int | None = None) -> None:
    css = hpc.css_variables_block(unit_id)
    st.markdown(
        f"""
<style>
{css}
.hp-screen {{ padding: var(--hc-spacing-screen) 0; }}
.hp-concept-name {{
  font-size: 1.45rem; color: var(--hc-text-primary); font-weight: 600; margin-bottom: 0.75rem;
}}
.hp-section-label {{
  font-size: 0.78rem; text-transform: uppercase; letter-spacing: 0.06em;
  color: var(--hc-text-secondary); margin: 1.1rem 0 0.35rem;
}}
.hp-body {{ color: var(--hc-text-primary); line-height: 1.55; font-size: 1.02rem; }}
.hp-remember {{
  background: var(--hc-bg-muted); padding: 0.85rem 1rem; border-radius: var(--hc-radius);
  border-left: 3px solid var(--hc-accent-teal); margin-top: 0.75rem;
}}
.hp-confusion {{
  background: #FFFBEB; padding: 0.85rem 1rem; border-radius: var(--hc-radius);
  border-left: 3px solid #F59E0B; margin-top: 0.75rem; color: #92400E;
}}
.hp-feedback {{
  background: var(--hc-feedback-amber); color: var(--hc-feedback-amber-text);
  padding: 1rem 1.25rem; border-radius: var(--hc-radius); margin-top: 1rem;
  border: 1px solid #E8D4A8; line-height: 1.5;
}}
.hp-diagram-wrap {{
  background: var(--hc-bg-surface); border: 1px solid var(--hc-border-subtle);
  border-radius: var(--hc-radius); padding: 0.75rem; margin: 1rem 0;
}}
.hp-glossary {{
  font-size: 0.92rem; color: var(--hc-text-secondary);
}}
.hp-diagram-caption {{
  font-size: 0.85rem; color: var(--hc-text-secondary); text-align: center; margin-top: 0.35rem;
}}
</style>
""",
