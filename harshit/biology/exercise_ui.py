"""Harshit Biology — Stage 3 NCERT written exercises."""
...
_TYPE_LABELS = {
    ...
    "translate_balance": "Translate & balance",
...
def _ss_key(name: str, unit_id: int) -> str:
    return f"hb_ex_u{unit_id}_{name}"


def _checklist_key(question_id: str, unit_id: int) -> str:
    return f"hb_ex_check_{unit_id}_{question_id}"
...
def render_stage3_home(*, stage1_done: bool, unit_id: int = 1) -> None:
    ...
    st.caption(
        ...
        "Attempt on paper first, then use hints and model answers."
    )
    ...
    for q in questions:
        ...
        with st.container(border=True):
            ...
            if len(preview) > 120:
                preview = preview[:117] + "…"
            st.caption(preview)
            if st.button("Open question", key=f"hb_ex_open_{unit_id}_{qid}", type="primary"):
                st.session_state.hb_unit_id = unit_id
                st.session_state.hb_exercise_id = qid
                st.session_state.current_page = "harshit_biology_exercise"
                st.rerun()
...
def render_exercise_question() -> None:
    ...
    qid = str(st.session_state.get("hb_exercise_id", ""))
    ...
    if hints:
        ...
        with col_h1:
            if hint_level < len(hints) and st.button(
                ...
                key=f"hb_ex_hint_{unit_id}_{qid}",
            ):
                st.session_state[_hint_level_key(qid, unit_id)] = hint_level + 1
                st.rerun()
        with col_h2:
            if hint_level > 0 and st.button("Reset hints", key=f"hb_ex_hint_reset_{unit_id}_{qid}"):
    ...
    if st.button(
        ...
        key=f"hb_ex_answer_{unit_id}_{qid}",
    ):
    ...
    if marking:
        ...
        for i, point in enumerate(marking):
            cid = f"{qid}_{i}"
            checks[cid] = st.checkbox(point, value=bool(checks.get(cid)), key=f"hb_ex_ck_{unit_id}_{cid}")
        state["checks"] = checks
    ...
    if concept_ids:
        st.markdown("---")
        if st.button("Review related concept", key=f"hb_ex_concept_{unit_id}_{qid}"):
