"""Streamlit UI — Harshit Biology, Unit 1 (Life Processes)."""
...
def _open_biology_home():
    st.session_state.current_page = "harshit_biology_home"


def _unit_id() -> int:
    return int(st.session_state.get("hb_unit_id", hpc.UNIT_ID))


def _open_unit_home(unit_id: int) -> None:
    st.session_state.hb_unit_id = unit_id
...
def _all_viewed_ids(unit_id: int | None = None) -> set[str]:
    ...
    if not user:
        return set()
    return set(db.get_harshit_biology_viewed_concepts(user["id"], unit_id=uid))
...
def _persist_concept_view(concept_id: str, *, marked_review: bool, unit_id: int | None = None) -> None:
    ...
    if not user:
        return
    db.save_harshit_biology_concept_status(
        user["id"],
        unit_id=uid,
...
def render_home() -> None:
    ...
    st.markdown(
        """
        <div style="background:#EEF1F5;padding:1.25rem;border-radius:8px;border-left:4px solid #16A34A;">
          <strong>Biology Confidence</strong> — understand and visualize before practice.
    ...
    if st.button("Unit 2 — Control and Coordination", use_container_width=True):
        _open_unit2_home()
        st.rerun()

    if st.button("Unit 3 — How do Organisms Reproduce?", use_container_width=True):
        _open_unit3_home()
        st.rerun()

    if st.button("Unit 4 — Heredity", use_container_width=True):
        _open_unit4_home()
        st.rerun()
...
def render_unit_home(unit_id: int) -> None:
    ...
    pdf_name = umeta.get("pdf", "jesc105.pdf")
    ...
    user = db.get_user("Harshit Sai")
    review_ids = db.get_harshit_biology_review_concepts(user["id"], unit_id=unit_id) if user else []
    ...
    section_key = f"hb_unit{unit_id}_section"
...
def _render_unit_learn(viewed: set[str], review_ids: list[str], *, unit_id: int) -> None:
    ...
    for day in hpc.list_days(stage=1, unit_id=unit_id):
        ...
        with cols[0]:
            ...
            st.caption(progress)
        with cols[1]:
            if active and st.button("Open", key=f"hb_open_u{unit_id}_day_{day_id}"):
    ...
    if review_ids:
        ...
        with st.expander("Cards marked for review"):
            for cid in review_ids:
                ...
                if concept and st.button(f"Review: {concept['name']}", key=f"hb_rev_u{unit_id}_{cid}"):
                    day = next(
                        d
...
def _render_day_menu(day_id: int, day: dict, *, unit_id: int) -> None:
    ...
    pick = st.selectbox("Choose a concept", options, label_visibility="collapsed", key=f"hb_pick_u{unit_id}_{day_id}")
...
def render_concept_card() -> None:
    ...
    umeta = hpc.unit_meta(unit_id)
    hpco.inject_biology_styles(unit_id)
    day_id = st.session_state.get("hb_day_id", 1)
    ...
    show_menu = st.session_state.get("hb_show_day_menu") == day_id
    ...
    if st.button(btn_label, type="primary", use_container_width=True):
        ...
        if is_last:
            user = db.get_user("Harshit Sai")
            if user:
                db.update_harshit_biology_day_status(
                    user["id"],
                    unit_id=unit_id,
                    day_id=day_id,
                    status="complete",
