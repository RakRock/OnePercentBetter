"""Harshit Biology — practice session UI (15 MCQs, email on completion)."""
...
def ensure_week_config(unit_id: int = 1) -> dict:
    config = db.get_harshit_biology_week_config(unit_id)
    if config.get("topics"):
        return config
    starter = hpt.default_week_config(unit_id)
    db.save_harshit_biology_week_config(
        unit_id,
        starter["week_label"],
        starter["topics"],
    ...
    return db.get_harshit_biology_week_config(unit_id)
...
def _clear_setup_widget_state(unit_id: int) -> None:
    ...
    st.session_state.pop(f"hb_setup_label_{unit_id}", None)
    st.session_state.pop(f"hb_setup_grok_{unit_id}", None)
    st.session_state.pop(f"hb_setup_grok_mode_{unit_id}", None)
    for did in hpt.topics_for_unit(unit_id):
        st.session_state.pop(f"hb_setup_day_{unit_id}_{did}", None)
...
def _start_practice(*, user_id: int | None = None, unit_id: int | None = None) -> None:
    ...
    st.session_state.hb_unit_id = uid
    st.session_state.current_page = "harshit_biology_practice"
...
def render_practice_home(*, stage1_done: bool, unit_id: int = 1) -> None:
    ...
    if st.button("Start practice", key=f"hb_start_practice_u{unit_id}", type="primary", use_container_width=True):
        uid = None
...
def render_setup_panel(unit_id: int = 1) -> None:
    current = db.get_harshit_biology_week_config(unit_id)
    stats = hpq.bank_stats(unit_id)
    ...
    if use_grok:
        grok_fresh_only = st.radio(
            ...
            key=f"hb_setup_grok_mode_{unit_id}",
        ) == "fresh"
    ...
    for did in sorted(topics_meta):
        ...
        picked = st.multiselect(
            ...
            default=default,
            key=f"hb_setup_day_{unit_id}_{did}",
        )
        levels = [level_options[p] for p in picked]
        if levels:
            new_topics.append({"id": did, "levels": levels})

    if st.button("Save practice focus", type="primary", key=f"hb_setup_save_{unit_id}"):
        db.save_harshit_biology_week_config(
            unit_id,
            week_label.strip(),
            new_topics,
    ...
    saved = db.get_harshit_biology_week_config(unit_id)
    if saved.get("topics"):
...
def _render_review(questions: list[dict], answers: list[dict], unit_id: int) -> None:
    ...
    if st.button("← Results", key=f"hb_review_back_u{unit_id}"):
    ...
    c1, c2 = st.columns(2)
    with c1:
        if idx > 0 and st.button("← Previous", key=f"hb_rev_prev_u{unit_id}"):
            st.session_state[_ss_key("review_index", unit_id)] = idx - 1
            st.rerun()
    with c2:
        if idx < total - 1 and st.button("Next →", key=f"hb_rev_next_u{unit_id}"):
            st.session_state[_ss_key("review_index", unit_id)] = idx + 1
            st.rerun()
...
def render_practice() -> None:
    ...
    hpco.inject_biology_styles(unit_id)
    name = st.session_state.get("selected_user")
    ...
    if st.button(f"← Unit {unit_id}"):
        st.session_state.current_page = f"harshit_biology_unit{unit_id}"
    ...
    if not is_done:
        ...
        for idx, opt in enumerate(q["options"]):
            with cols[idx % 2]:
                if st.button(str(opt), key=f"hb_u{unit_id}_choice_{current}_{idx}", use_container_width=True):
                    picked = idx
    ...
    if user and session_id and st.session_state.get(_ss_key("persist_saved_for", unit_id)) != session_id:
        ...
        try:
            _, sheet_err = gss.persist_edgenuity_practice(
                ...
                unit_label=f"Biology Unit {unit_id}: {umeta['title']} — {week_label}",
                report=report,
    ...
    if session_id and st.session_state.get(_ss_key("email_sent_for", unit_id)) != session_id:
        ...
        if ec3mail.practice_email_enabled():
            mail_result = ec3mail.send_harshit_report_email(
                ...
                unit_title=f"Biology Unit {unit_id}: {umeta['title']}",
    ...
    with c1:
        if st.button("🔍 Review answers", type="primary", use_container_width=True, key=f"hb_review_u{unit_id}"):
            st.session_state[_ss_key("review_mode", unit_id)] = True
            st.session_state[_ss_key("review_index", unit_id)] = 0
            st.rerun()
    with c2:
        if st.button("Practice again", use_container_width=True, key=f"hb_again_u{unit_id}"):
            uid = user["id"] if user else None
            _start_practice(user_id=uid, unit_id=unit_id)
            st.rerun()
    with c3:
        if st.button(f"← Unit {unit_id} home", use_container_width=True, key=f"hb_home_u{unit_id}"):
            st.session_state.current_page = f"harshit_biology_unit{unit_id}"
            st.session_state[_ss_key("questions", unit_id)] = []
            st.rerun()
