"""Harshit Biology — Stage 2 structured MCQ sessions (days 17–20)."""
...
def _open_concept_by_id(concept_id: str, unit_id: int) -> None:
    ...
    key = f"hb_concept_{unit_id}_{day_id}"
    if key not in st.session_state:
        st.session_state[key] = hcs.ConceptSessionState(day_id=day_id).to_dict()


def _get_mcq_state(day_id: int, unit_id: int) -> hcs.MCQSessionState:
    key = _mcq_state_key(day_id, unit_id)
    return hcs.MCQSessionState.from_dict(st.session_state.get(key, {"day_id": day_id}))


def _save_mcq_state(state: hcs.MCQSessionState, unit_id: int) -> None:
...
def render_stage2_home(*, stage1_done: bool, unit_id: int = 1) -> None:
    ...
    if pdf_name:
        ...
        if pdf_path.is_file():
            ch = ncert.get("meta", {}).get("chapter", unit_id)
            with st.expander(f"📄 NCERT Chapter {ch} PDF (Activities & questions)", expanded=False):
                try:
                    st.pdf(pdf_path)
                except Exception:
    ...
    for session in hpc.list_mcq_sessions(unit_id):
        ...
        with st.container(border=True):
            ...
            if st.button("Start session", key=f"hb_stage2_start_{unit_id}_{day_id}", type="primary"):
                st.session_state.hb_unit_id = unit_id
                st.session_state.hb_mcq_day_id = day_id
                ...
                _save_mcq_state(hcs.MCQSessionState(day_id=day_id), unit_id)
                st.session_state.current_page = "harshit_biology_mcq"
                st.rerun()
...
def render_mcq_session() -> None:
    ...
    day_id = int(st.session_state.get("hb_mcq_day_id", 17))
    ...
    for idx, opt in enumerate(q["options"]):
        with cols[idx % 2]:
            if st.button(str(opt), key=f"hb_mcq_u{unit_id}_d{day_id}_q{current}_{idx}", use_container_width=True):
                picked = idx
...
def _render_remediation(payload: dict, unit_id: int, day_id: int, state: hcs.MCQSessionState) -> None:
    q = payload["question"]
    ...
    if concept_id:
        if st.button("Review related concept", key=f"hb_mcq_review_concept_{unit_id}"):
            _open_concept_by_id(concept_id, unit_id)
            st.session_state.pop(_ss_key("remediation", unit_id), None)
            st.rerun()

    if st.button("Continue to next question →", type="primary", key=f"hb_mcq_continue_{unit_id}"):
        st.session_state.pop(_ss_key("remediation", unit_id), None)
        current = int(st.session_state.get(_ss_key("current", unit_id), 0))
        st.session_state[_ss_key("current", unit_id)] = current + 1
        st.rerun()
