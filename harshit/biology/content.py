"""Harshit Biology — multi-unit content loader (NCERT Class 10)."""
...
UNITS: dict[int, dict] = {
    ...
    2: {
        ...
        "ncert": "Class 10 Science, Chapter 6 (Biology)",
        "session_kind_concepts": "harshit_biology_unit2_concepts",
        "session_kind_mcq": "harshit_biology_unit2_mcq",
    ...
    3: {
        ...
        "ncert": "Class 10 Science, Chapter 7 (Biology)",
        "session_kind_concepts": "harshit_biology_unit3_concepts",
        "session_kind_mcq": "harshit_biology_unit3_mcq",
    ...
    4: {
        "dir": "unit4",
        "title": "Heredity",
        "ncert": "Class 10 Science, Chapter 8 (Biology)",
        "session_kind_concepts": "harshit_biology_unit4_concepts",
        "session_kind_mcq": "harshit_biology_unit4_mcq",
...
SESSION_UNIT_OFFSET = 500

UNIT_ID = 1
UNIT_TITLE = UNITS[1]["title"]
UNIT1_DIR = ROOT / "HarshitBiology" / "unit1"
...
def unit_dir(unit_id: int) -> Path:
    if unit_id not in UNITS:
        raise ValueError(f"Unknown biology unit_id: {unit_id}")
    return ROOT / "HarshitBiology" / UNITS[unit_id]["dir"]
...
def active_unit_id() -> int:
    try:
        ...
        return int(st.session_state.get("hb_unit_id", UNIT_ID))
    except Exception:
        return UNIT_ID
...
def _testing_practice_unlock_enabled() -> bool:
    ...
    if os.environ.get("HARSHIT_BIOLOGY_UNLOCK_PRACTICE", "").strip().lower() in ("1", "true", "yes"):
        return True
    try:
        import streamlit as st

        return bool(st.secrets.get("HARSHIT_BIOLOGY_UNLOCK_PRACTICE"))
    except Exception:
        return False


def practice_unlocked(viewed_ids: set[str], unit_id: int | None = None) -> bool:
    """Stage 1 complete, or testing bypass via HARSHIT_BIOLOGY_UNLOCK_PRACTICE."""
